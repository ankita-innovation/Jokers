...
extension BPStatusBarAlert {
    ...
    fileprivate func frame(position: Position) -> CGRect {
        ...
        case .statusBar:
            if UIDevice.current.hasNotch{
                ...
                frame = CGRect(x: 0, y: -statusBarHeight, width: screenWidth, height: statusBarHeight+50)
            }
        case .navigationBar:
            if UIDevice.current.hasNotch{
                frame = CGRect(x: 0, y: navigationBarHeight, width: screenWidth, height: statusBarHeight)
            }else{
                frame = CGRect(x: 0, y: navigationBarHeight, width: screenWidth, height: statusBarHeight+50)
            }
        }
        return frame
    }
    ...
    fileprivate func setupMessageLabel() {
        if UIDevice.current.hasNotch{
            ...
            messageLable.frame = CGRect(x: 10, y: deviceStatusBarHeight+5, width: frame.size.width - 10, height: statusBarHeight)
        }
        messageLable.textColor = messageColor
        messageLable.textAlignment = .center
        messageLable.numberOfLines = 0
        messageLable.font = small
