...
final class HLSVideoCache {
    ...
    private let contentType = "application/vnd.apple.mpegurl"
    private let port: UInt = 1345
    ...
    private init() {
        ...
        // 200 mb disk cache
        let diskConfig = DiskConfig(name: "HLS_Video", expiry: .never, maxSize: 500 * 1024 * 1024)
...
//                    }
    ...
    private func cacheKey(for resourceURL: URL) -> String {
        ...
        if #available(iOS 13.0, *) {
            return SHA256.hash(data: Data(resourceURL.absoluteString.utf8)).map { String(format: "%02hhx", $0) }.joined()
        } else {
            // Fallback on earlier versions
            let data = ccSha256(data: resourceURL.absoluteString.data(using: .utf8)!)
