...
public func config(color:UIColor,font:UIFont?, align:NSTextAlignment,place:String){
    self.textColor = color
    self.tintColor = PRIMARY_COLOR
    self.textAlignment = align
    self.placeholder = Utility.shared.getAppLanguage()?.value(forKey: place) as? String
    self.font = font
    }
    //MARK: Rounded radius
